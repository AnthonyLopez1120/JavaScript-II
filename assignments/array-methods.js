...
let allNames=[];
allNames=runners.map(function(item){ 
...
// Problem 1 The Event was canceled due to rain and needs to be rescheduled. Create an list of emails so we can notify the runners of the new time

let emails = [];

runners.forEach(function(items){
  return emails.push(`${items.email}`);
})
console.log(emails);


// Problem 2 We need the Company Names to be displayed in UPPPER CASE for brand recognition. 

let companyName=[];
companyName=runners.map(function(item){ 
...
// Problem 3 Two participants dropped out of the race. Update the list to remove Shell and Bell

let droppedOut = [];

droppedOut=runners.filter(function(item){

  return item.first_name !== 'Shell' || 'Bell';
})

console.log(droppedOut);
