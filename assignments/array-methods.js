...
let firstNamesAllCaps = runners.map(curVal=>{
  return curVal.first_name.toUpperCase();
});
...
runnersLargeSizeShirt = runners.filter(curVal=>{
  return curVal.shirt_size === 'L';
  });
...
ticketPriceTotal = runners.reduce(function(acc, curVal){
  return acc + curVal.donation;
},ticketPriceTotal);
...
// Problem 1 The event had to be rescheduled for rain. Create a new array of emails to send a notice out.

let emails = [];

runners.forEach(curVal=>{ 
  emails.push(curVal.email);

});
console.log(emails);

// Problem 2 One of the racers dropped out. We need an updated roster of participants. 

let newRunners = [];

newRunners = runners.filter(curVal=>{
  return curVal.first_name !== "Shell"
});
...
// Problem 3 To increase brand recognition, we need to capitalize the company names

const companyNames = runners.map(curVal=>{
  return curVal.company_name.toUpperCase();
})
